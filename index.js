...
io.on('connection', function (socket) {
    ...
    socket.on('Control-Web', function (data) {
        if (data) {
            ...
            // Encontrar ultimo registro de la variable
            Val_Variables.find({ "id_usuario": id_usuario, "id_variable": id_variable }, function (err, doc) {
                if (err) {
                    console.log(err)
                    return false
                }
                if (doc) {
                    if (parseInt(doc.length) > 0) {
                        var id_ultimo = doc[0]._id;
                        // Actualizar el registro en la base de datos
                        Val_Variables.findOne({ '_id': id_ultimo }, function (err2, doc2) {
                            if (err2) {
                                console.log(err)
                                return false
                            }
                            if (doc2) {
                                doc2.fecha = fecha;
                                doc2.val = val;
                                doc2.save(function (err3, result3) {
                                    if (err3) {
                                        console.log(err3);
                                        return false;
                                    }
            ...
            }).limit(1).sort({ $natural: -1 }).select({ _id: 1 }).lean();
    ...
    // Evento socket: Consultar registrador
    socket.on('Consultar-Registrador', function (data) {
        console.log("Rgistrador", data);
        ...
        Consultar_Registrador(id_usuario, id_widget, id_variable, fecha_inicio, fecha_final, socket.id)
    });
    ...
    socket.on('Consulta', function (id_usuario, bd, Fecha_Inicio, Fecha_Final) {
        ...
        if ((bd == "usuarios") && (id_usuario == "1")) {
            ...
            Tableros.find({ "id_usuario": id_usuario }, function (err, doc) {
                if (err) return false
                if (doc) {
                    var longitud = parseInt(doc.length);
                    var inicio = 0;
                    ...
                    for (var i = inicio; i < longitud; i++) {
                        ...
                        var fecha = doc[i].fecha;
                        fecha = new Date(fecha).toLocaleString("en-GB", formato_fecha);
                        io.sockets.in(socket.id).emit('Tableros', final_trama, {
                            id: doc[i]._id,
                            fecha: fecha,
                            nombre: doc[i].nombre,
                        });
    ...
    socket.on('Generar_Reporte_Variable', function (data) {
        ...
        if (month < 10) {
            ...
            fecha_reporte2 = "Fecha: " + `${day}-${month}-${year}`
        }

        if (tipo == 'PDF') {
            console.log("Se generara un PDF");
            nombre_archivo = fecha_reporte + " " + titulo_reporte + ".pdf";
            header_nombre = nombre_variable;
        } else if (tipo == 'CSV') {
            console.log("Se generara un CSV");
            ...
            console.log("Reporte sin especificacion de archivo");
        ...
        Val_Variables.find({ "id_usuario": id_usuario, "id_variable": id_variable, "fecha": { "$gte": fecha_inicio, "$lte": fecha_final } }, function (err, doc) {
            if (err) {
                console.log("Error consultando variables para generar PDF")
                io.sockets.in(socket.id).emit('reporte-error', { id_widget });
                return false;
            }
            if (doc) {
                ...
                if (tipo == "PDF") {
                    Usuarios.findById(id_usuario, function (err2, doc2) {
                        if (err2) {
                            console.log("Error consultando logo del usuario")
                            io.sockets.in(socket.id).emit('reporte-error', { id_widget });
                            return false;
                        }
                        if (doc2) {
                            var logo = doc2.logo;
                            var aux_logo = 'public/logos/' + logo;

                            if (!fs.existsSync(aux_logo)) {
                                console.log("Logo para PDF no encontrado.");
                                aux_logo = 'public/logos/logo.png';
                            }
                            var path_url = path.join(__dirname, aux_logo),
                                format = 'PNG';
                            ...
                            console.log("No hay data del usuario para encontrar su logo")
                            io.sockets.in(socket.id).emit('reporte-error', { id_widget });
                        }
                    }).select({ logo: 1 }).lean();
                }
                if (tipo == "CSV") {
                    csvWriter.writeRecords(lista2).then(() => {
                        fs.rename(oldPath, newPath, function (err) {
                            if (err) {
                                console.log("Error guardando CSV")
                                io.sockets.in(socket.id).emit('reporte-error', { id_widget });
                                return false;
                            }
                            io.sockets.in(socket.id).emit('reporte-enviado', { id_widget, nombre_archivo });
...
app.get('/', (req, res, next) => {
    ...
    res.redirect("/tableros"); // Si ya ha iniciado sesion
});
...
app.get('/registro-estancia', (req, res, next) => {
    ...
    if (datos.permiso_edicion == 0) {
        ...
        Tableros.find({ "id_usuario": id }, function (err1, doc1) {
            if (err1) return false
            if (doc1) {
                var id_tableros = [],
                    tableros = [];
                var cantidad_tableros = parseInt(doc1.length);
                for (var i = 0; i < cantidad_tableros; i++) {
                    id_tableros[i] = doc1[i]._id;
                    tableros[i] = doc1[i].nombre;
                }
                res.render('registro-estancia.html', {
                    datos,
                    cantidad_tableros,
                    id_tableros,
                    tableros
...
app.get('/registro-widget', (req, res, next) => {
    ...
    if (datos.permiso_edicion == 1) {
        Estancias.find({ "id_usuario": id }, function (err, doc) {
            ...
            if (doc) {
                ...
                for (var i = 0; i < cantidad_estancias; i++) {
                    ...
                    id_tablero_estancias[i] = doc[i].id_tablero;
                    tablero_estancias[i] = doc[i].tablero;
                }
                Variables.find({ "id_usuario": id }, function (err1, doc1) {
                    ...
                    if (doc1) {
                        ...
                        Tableros.find({ "id_usuario": id }, function (err2, doc2) {
                            if (err2) return false
                            if (doc2) {
                                var cantidad_tableros = parseInt(doc2.length);
                                var id_tableros = [],
                                    tableros = [];
                                for (var i = 0; i < cantidad_tableros; i++) {
                                    id_tableros[i] = doc2[i]._id;
                                    tableros[i] = doc2[i].nombre;
                                }
        ...
        }).select({ _id: 1, nombre: 1, id_tablero: 1, tablero: 1 }).lean();
    } else {
        res.render("sin-permiso-edicion.html", {
            datos
        });
    ...
    if (datos.permiso_edicion == 1) {
        Variables.find({ "id_usuario": id }, function (err1, doc1) {
            ...
            if (doc1) {
                ...
                Widgets.find({ "id_usuario": id, "tipo": "Led" }, function (err2, doc2) {
                    if (err2) return false
                    if (doc2) {
                        var cantidad_widgets_led = parseInt(doc2.length);
                        var id_widgets_led = [],
                            widgets_led = [];
                        for (var i = 0; i < cantidad_widgets_led; i++) {
                            id_widgets_led[i] = doc2[i]._id;
                            widgets_led[i] = doc2[i].nombre;
                        }
...
// Ruta: Lista de eventos po horario registrados por usuario 
app.get('/eventos_horario', (req, res, next) => {
    if (req.isAuthenticated()) {
        var id = req.session.passport.user.id;
        console.log("id: " + id)
        if (id != 1)
    ...
    var id = req.session.passport.user.id;
    if (datos.permiso_edicion == 1) {
        Widgets.find({ "id_usuario": id, "tipo": "Programador_horario" }, function (err1, doc1) {
            if (err1) return false
            if (doc1) {
                var cantidad = parseInt(doc1.length);
                var id_widget = [],
                    id_variables = [],
                    variables = [],
                    dias_horario = [],
                ...
                for (var i = 0; i < cantidad; i++) {
                    id_widget[i] = doc1[i]._id;
                    id_variables[i] = doc1[i].id_variable;
                    variables[i] = doc1[i].variable;
                    dias_horario[i] = doc1[i].dias_horario;
                    hora_horario[i] = doc1[i].hora_horario;
                    duracion_horario[i] = doc1[i].duracion_horario;
                }
                console.log(id_variables, variables, dias_horario, hora_horario, duracion_horario, dia)
                res.render('eventos_horario.html', {
                    datos,
                    cantidad,
                    id_widget,
                    id_variables,
        ...
        }).select({ _id: 1, id_variable: 1, variable: 1, dias_horario: 1, hora_horario: 1, duracion_horario: 1 }).lean();
    } else {
        res.render("sin-permiso-edicion.html", { datos });
    }
});
// Ruta: Registro de tablero
app.get('/registro-tablero', (req, res, next) => {
    if (req.isAuthenticated()) {
        var id = req.session.passport.user.id;
        console.log("id: " + id)
        if (id != 1)
    ...
    var datos = req.session.passport.user;
    var html_render = "registro-tablero.html";
    if (datos.permiso_edicion == 0)
        html_render = "sin-permiso-edicion.html";
    res.render(html_render, {
        datos
...
// Ruta: Lista de tableros registrados por usuario 
app.get('/tableros', (req, res, next) => {
    if (req.isAuthenticated()) {
        var id = req.session.passport.user.id;
        console.log("id: " + id)
        if (id != 1)
    ...
    var datos = req.session.passport.user;
    var html_render = "tableros.html";
    res.render(html_render, {
        datos
    }); // Si ya inicio sesion
});
// Ruta: Tablero o dashboard
app.get('/tableros/tablero', (req, res, next) => {
    if (req.isAuthenticated()) {
        var id = req.session.passport.user.id;
        console.log("id: " + id)
        if (id != 1)
    ...
    var id_tablero = req.query.tablero;
    console.log("tablero:", id_tablero);
    // Datos del tablero
    console.log("Buscando estancias");
    Estancias.find({ "id_usuario": id, "id_tablero": id_tablero }, function (err1, doc1) {
        if (err1) return false
        if (doc1) {
            var id_estancias = [],
                estancias = [];
            ...
            Variables.find({ "id_usuario": id }, function (err2, doc2) {
                ...
                if (doc2) {
                    ...
                    Widgets.find({ "id_usuario": id, "id_tablero": id_tablero }, function (err3, doc3) {
                        ...
                        if (doc3) {
                            ...
                            var msj_off_widgets = [],
                                msj_on_widgets = [],
                                funcion_widgets = [],
                                tiempo_boton_widgets = [];
                            var titulos_tablas_widgets = {},
                            ...
                            Tableros.findById(id_tablero, function (err3, doc3) {
                                if (err3) return false
                                if (doc3) {
                                    var nombre_tablero = doc3.nombre;
...
app.post('/datos-registro-bd', (req, res) => {
    ...
    if (req.isAuthenticated()) {
        ...
        if (bd == "usuarios") {
            ...
            var id_tablero = req.body.id_tablero;
            var tablero = req.body.tablero;
            var nombre = req.body.nombre;
            var ubicacion = req.body.ubicacion;
            Estancias.findOne({ 'id_usurio': id_usuario, 'estancia': nombre, 'tablero': tablero }, function (err, doc) {
                if (err) {
                    res.send("Error");
                    return false;
                }
            ...
            // Programador hoario
            var dias_horario = req.body.string_dias_horario;
            var hora_horario = req.body.string_hora_horario;
            var duracion_horario = req.body.string_duracion_horario;
            dias_horario = dias_horario.split(',');
            hora_horario = hora_horario.split(',');
            duracion_horario = duracion_horario.split(',');
            var array_dias_horario = [],
                array_hora_horario = [],
                array_duracion_horario = [];
            for (var i = 1; i <= 7; i++) {
                array_dias_horario[i] = dias_horario[i];
                array_hora_horario[i] = hora_horario[i];
                array_duracion_horario[i] = duracion_horario[i];
            }
            ...
            Widgets.findOne({ 'id_usuario': id_usuario, 'id_estancia': id_estancia, 'nombre': nombre }, function (err, doc) {
                ...
                if (doc) { // Si ya esta registrado
                    ...
                    var widgets = new Widgets({
                        ...
                        msj_off,
                        msj_on,
                        dias_horario: array_dias_horario,
                        hora_horario: array_hora_horario,
                        duracion_horario: array_duracion_horario,
                        funcion,
                        id_estancia,
                        estancia,
                        color
            ...
            var nombre = req.body.nombre;
            Tableros.findOne({ 'id_usuario': id_usuario, 'nombre': nombre }, function (err, doc) {
                if (err) {
                    res.send("Error");
                    return false;
                }
                if (doc) { // Si ya esta registrado
                    ...
                    var tableros = new Tableros({
                        ...
                        var aux_id_tablero = result1._id;
                        var estancias = new Estancias({
                            fecha,
                            id_usuario,
                            id_tablero: aux_id_tablero,
                            tablero: nombre,
                            nombre: "...",
                            ubicacion: ""
                        }).save(function (err2) {
...
// Editar variable
app.post('/datos-editar-variable', (req, res) => {
    console.log(req.body)
    var id_variable = req.body.id;
    var nombre_variable = req.body.nombre;
    var unidad_med = req.body.unidad_med;

    if (req.isAuthenticated()) {
        var id = req.session.passport.user.id;
        Variables.findById(id_variable, function (err, doc) {
            if (doc) {
                console.log("variable a modificar:", doc)
                doc.nombre = nombre_variable
                doc.unidad_med = unidad_med;
                doc.save(function (err, result) {
                    if (err) {
                        console.log(err);
                        res.send({ mensaje: "Error" })
                    ...
                    Widgets.updateMany({ id_variable: id_variable }, { $set: { variable: nombre_variable } }, function (err, doc) {
                        if (err) {
                            console.log(err);
                            res.send({ mensaje: "Error" })
                            return false;
                        }
                        console.log("Variables en widgets modificadas")
                        console.log("Modificado");
                        res.send({ mensaje: "Modificado" })
                    });
                });
...
// Editar widget
app.post('/datos-editar-widget', (req, res) => {
    console.log(req.body)
    var id_widget = req.body.id;
    var nombre_widget = req.body.nombre;

    if (req.isAuthenticated()) {
        Widgets.findById(id_widget, function (err, doc) {
            if (doc) {
                console.log("Widget a modificar:", doc)
                doc.nombre = nombre_widget
                doc.save(function (err, result) {
                    if (err) {
                        console.log(err);
                        res.send({ mensaje: "Error" })
...
app.post('/eliminar-registro-bd', (req, res) => {
    ...
    if (req.isAuthenticated()) {
                        ...
                        Variables.deleteMany({ 'id_usuario': id }, function (err4) {
                            ...
                            Widgets.deleteMany({ 'id_usuario': id }, function (err5) {
                                ...
                                Val_Variables.deleteMany({ 'id_usuario': id }, function (err6) {
                                    ...
                                    Tableros.deleteMany({ 'id_usuario': id }, function (err7) {
                                        if (err7) {
                                            res.send("Error");
                                            return false;
                                        }
        ...
        if (bd == "variables") {
            Variables.deleteOne({ '_id': id }, function (err) {
                ...
                Val_Variables.deleteMany({ 'id_variable': id }, function (err2) {
                    ...
                    Widgets.deleteMany({ 'id_variable': id }, function (err3) {
                        if (err3) {
                            res.send("Error");
                            return false;
                        }
        ...
        if (bd == "tableros") {
            Tableros.deleteOne({ '_id': id }, function (err) {
                if (err) {
                    res.send("Error");
                    return false;
                }
                Widgets.deleteMany({ 'id_tablero': id }, function (err2) {
                    if (err2) {
                        res.send("Error");
                        return false;
                    }
                    Estancias.deleteMany({ 'id_tablero': id }, function (err3) {
                        if (err3) {
                            res.send("Error");
                            return false;
                        }
...
// Variables de la base de datos para los tableros
var user_schema9 = new mongoose.Schema({
    id_usuario: String,
    fecha: Date,
    nombre: String
}, { versionKey: false });
var Tableros = mongoose.model("tableros", user_schema9);
...
function Consultar_Registrador(id_usuario, id_widget, id_variable, fecha_inicio, fecha_final, id_webapp) {
    io.sockets.in(id_webapp).emit('Limpiar-Grafica', { id_widget });
    ...
    Val_Variables.find({ "id_usuario": id_usuario, "id_variable": id_variable, "fecha": { "$gte": fecha_inicio, "$lte": fecha_final } }, function (err, doc) {
        ...
        if (doc) {
            ...
            io.sockets.in(id_webapp).emit('Cantidad_Datos_Registrador', {
                id_widget,
                cantidad_datos
            });
        }
...
client.on('connect', () => {
    ...
    client.subscribe(suscripcion_topic_actualizar, function (err) {
        if (err) {
            console.log("Error al suscribir a topic MQTT", err);
            return false;
        }
        console.log("Suscrito a: ", suscripcion_topic_actualizar);
    })
...
client.on('message', (topic, message) => {
    ...
    try {
        ...
        if (json_mensaje) {
            ...
            if (topic == suscripcion_topic_actualizar) {
                ///console.log('Topic>>', topic, ':', json_mensaje)
                var longitud_datos = Object.keys(json_mensaje).length;
                var parametros_datos = Object.keys(json_mensaje);
                ...
                for (var i = 0; i < longitud_datos; i++) {
                    if ((parametros_datos[i] != "token") && (parametros_datos[i] != "id_dispositivo")) {
                        ...
                        Verificar_Evento(id_usuario, id_dispositivo, id_variable);
                        // Registrar valores en base de datos
                        Val_Variables.findOne({ 'id_variable': id_variable }, function (err2, doc2) {
                            if (err2) {
                                return false;
                            }
                            if (doc2) {
                                doc2.fecha = fecha;
                                doc2.val = valor_variables[doc2.id_variable];
                                doc2.save();
                            }
                        });
                    }
            ...
            if (aux_evento == 1) {
                ...
                if (estado_evento == 1) {
                    ...
                    for (var a = 0; a < cantidad_acciones; a++) {
                        ...
                        // Control de led
                        if (acciones[a].tipo == 2) {
                            var id_widget = acciones[a].id_widget_led;
                            var val_color = acciones[a].color_led;
                            console.log("Control led>> ID widget:", id_widget, "Color:", val_color);
                            // Actualizar en base de datos el color del widget
                            Widgets.findOne({ 'id_usuario': id_usuario, '_id': id_widget }, function (err, doc) {
                                if (doc) {
                                    doc.color = val_color;
                            ...
                            // Enviar a todos los dashboard del usuario
                            var json_datos_tablero = {};
                            json_datos_tablero["id_widget"] = id_widget;
                            json_datos_tablero["color"] = val_color;
                            io.sockets.in(id_usuario).emit('Color_Widget', json_datos_tablero);
                        }
                    }
                }
            }
...
var variables_dispositivos = {}; // Para guardar el id del dispositivo de cada variable
        ...
        Variables.find(function (err, doc) {
            ...
            if (doc) {
                ...
                for (var i = 0; i < cantidad_variables; i++) {
                    ...
                    variables_dispositivos[id_variable] = doc[i].id_dispositivo;
...
Consultar_Eventos();
var cantidad_programador_horario = 0,
    objeto_dias_horario = {},
    objeto_hora_horario = {},
    objeto_duracion_horario = {},
    objeto_ms_duracion_horario = {},
    objeto_aux_estado = {};
var array_id_programador_horario = [],
    objeto_id_variable_programador_horario = {},
    objeto_estado_progaramador_horario = {};
var listo_horario = 0;
// Funcion: Consultar en la bd los horarios de activacion
function Consultar_Horarios_BD() {
    setTimeout(function () {
        Consultar_Horarios_BD();
        Widgets.find({ tipo: "Programador_horario" }, function (err, doc) {
            if (err) return false;
            if (doc) {
                var cantidad_datos = parseInt(doc.length);
                cantidad_programador_horario = cantidad_datos;
                for (i = 0; i < cantidad_datos; i++) {
                    var id_programador_horario = doc[i]._id;
                    array_id_programador_horario[i] = id_programador_horario;
                    objeto_id_variable_programador_horario[id_programador_horario] = doc[i].id_variable;
                    objeto_dias_horario[id_programador_horario] = doc[i].dias_horario;
                    objeto_hora_horario[id_programador_horario] = doc[i].hora_horario;
                    objeto_duracion_horario[id_programador_horario] = doc[i].duracion_horario;
                    listo_horario = 1;
                }
            }
        }).select({ _id: 1, id_variable: 1, dias_horario: 1, hora_horario: 1, duracion_horario: 1 }).lean();
    }, 30000, "JavaScript");
};
Consultar_Horarios_BD();

// Funcion: Verificar horarios de activacion
function Verificar_Horarios() {
    setTimeout(function () {
        Verificar_Horarios();
        if (listo_horario == 1) {
            for (var i = 0; i < cantidad_programador_horario; i++) {
                var dia_actual = new Date().getDay() + 1;
                var hora_actual = new Date().getHours();
                var minuto_actual = new Date().getMinutes();
                //console.log("Dia actual:", dia_actual, "Hora actual:", hora_actual, "Minuto actual:", minuto_actual);
                var id_programador_horario = array_id_programador_horario[i];
                var id_variable = objeto_id_variable_programador_horario[id_programador_horario];
                // Obtener dias, hora y minuto
                var dias = objeto_dias_horario[id_programador_horario]; // Array dias
                var estado_dia = dias[dia_actual]; // Estado del dia actual
                var aux_hora = objeto_hora_horario[id_programador_horario]; // Array hora
                var aux_hora2 = aux_hora[dia_actual].split(":"); // Obtener hora del dia actual
                var hora = parseInt(aux_hora2[0]); // Hora
                var minuto = parseInt(aux_hora2[1]); // Minuto
                //console.log("Programador:", id_programador_horario, "Estado dia:", estado_dia, "Hora:", hora, "Minuto:", minuto);
                // Verificar si estamos en el dia de activacion
                if (estado_dia == '1') {
                    // Estamos en la misma hora
                    if (hora == hora_actual) {
                        // Estamos en los mismos minutos
                        if (minuto == minuto_actual) {
                            if ((objeto_estado_progaramador_horario[id_programador_horario] != 1) && (objeto_aux_estado[id_programador_horario] != 1)) {
                                // Actualizar objeto
                                valor_variables[id_variable] = 1;
                                objeto_estado_progaramador_horario[id_programador_horario] = 1;
                                var dur = parseInt(objeto_duracion_horario[id_programador_horario][dia_actual]) * 1000;
                                var ms_actuall = parseInt(new Date().getTime());
                                objeto_ms_duracion_horario[id_programador_horario] = dur + ms_actuall;
                                console.log("Activar variable:", id_variable, " y programador:", id_programador_horario, " Duracion:", dur, " en", ms_actuall, " se desactiva:", objeto_ms_duracion_horario[id_programador_horario]);
                                // Encontrar ultimo registro de la variable
                                Val_Variables.find({ "id_variable": id_variable }, function (err, doc) {
                                    if (err) {
                                        console.log(err)
                                        return false
                                    }
                                        ...
                                        if (parseInt(doc.length) > 0) {
                                            ...
                                            Val_Variables.findOne({ '_id': id_ultimo }, function (err2, doc2) {
                                                ...
                                                if (doc2) {
                                                    doc2.fecha = new Date();
                                                    doc2.val = 1;
                                                    var id_usuario = doc2.id_usuario;
                                                    doc2.save(function (err3, result3) {
                                                        if (err3) {
                                                            console.log(err3);
                                                        }
                                                        var id_dispositivo = variables_dispositivos[id_variable];
                                                        // Enviar a todos los dashboard del usuario
                                                        var json_datos_tablero = {};
                                                        json_datos_tablero["id_variable"] = id_variable;
                                                        json_datos_tablero["val_variable"] = 1;
                                                        io.sockets.in(id_usuario).emit('Val_Variables', json_datos_tablero);
                                                        // Enviar al dispositivo por MQTT
                                                        var val_variable = {};
                                                        val_variable[id_variable] = 1;
                                                        val_variable["e-" + id_variable] = 1;
                                                        var topic_enviar = "server_iot/" + id_usuario + "/" + id_dispositivo;
                                                        client.publish(topic_enviar, JSON.stringify(val_variable));
                                                        console.log("Programador horario>> Enviado a :", topic_enviar, val_variable);
                                                        // Verificar eventos
                                                        Verificar_Evento(id_usuario, id_dispositivo, id_variable);
                                                    });
                                                }
                            ...
                            objeto_aux_estado[id_programador_horario] = 0;
                        }
                    } else {
                        objeto_aux_estado[id_programador_horario] = 0;
                    }
                ...
                // Verificar para desactivar
                if (objeto_estado_progaramador_horario[id_programador_horario] == 1) {
                    var ms_actual = parseInt(new Date().getTime());
                    if (ms_actual >= objeto_ms_duracion_horario[id_programador_horario]) {
                        // Actualizar objeto
                        valor_variables[id_variable] = 0;
                        objeto_estado_progaramador_horario[id_programador_horario] = 0;
                        objeto_aux_estado[id_programador_horario] = 1;
                        console.log("Desactivar variable:", id_variable, " y programador:", id_programador_horario), "Tiempo actual:", ms_actual;
                        // Encontrar ultimo registro de la variable
                        Val_Variables.find({ "id_variable": id_variable }, function (err, doc) {
                            if (err) {
                                console.log(err)
                                    ...
                                    Val_Variables.findOne({ '_id': id_ultimo }, function (err2, doc2) {
                                        ...
                                        if (doc2) {
                                            ...
                                            doc2.save(function (err3, result3) {
                                                ...
                                                var json_datos_tablero = {};
                                                json_datos_tablero["id_variable"] = id_variable;
                                                json_datos_tablero["val_variable"] = 0;
                                                io.sockets.in(id_usuario).emit('Val_Variables', json_datos_tablero);
                                                // Enviar al dispositivo por MQTT
                                                var val_variable = {};
                                                val_variable[id_variable] = 0;
                                                val_variable["e-" + id_variable] = 1;
                                                var topic_enviar = "server_iot/" + id_usuario + "/" + id_dispositivo;
                                                client.publish(topic_enviar, JSON.stringify(val_variable));
                                                console.log("Programador horario>> Enviado a :", topic_enviar, val_variable);
